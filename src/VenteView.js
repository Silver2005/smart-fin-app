...
const VenteView = ({ articles, onAddVente }) => {
  ...
  // On trouve l'article pour le formulaire (stock, etc.)
  ...
  const handleSubmit = (e) => {
    ...
    // On valide l'article une dernière fois pour le reçu
    ...
    // 🌟 MISE À JOUR : On fixe le nom de l'article ici explicitement
    setReceipt({ 
      ...saleData, 
...
Développé par SILVER'S DESIGN
  ...
  if (receipt) {
    return (
      <div className="flex flex-col items-center w-full animate-in zoom-in-95">
        {/* Style spécifique pour l'impression PDF propre */}
        <style>{`
          @media print {
        ...
        <div className="mt-8 space-y-3 w-full max-w-md no-print">
          {isMobile ? (
            <button 
              onClick={handleShare} 
              className="w-full py-4 bg-emerald-600 text-white font-black uppercase text-xs rounded-2xl active:scale-95 transition-transform shadow-lg shadow-emerald-900/40"
            >
            ...
            <button 
              ...
              className="w-full py-4 bg-slate-900 text-white font-black uppercase text-xs rounded-2xl hover:bg-black transition-all shadow-lg shadow-slate-900/40"
            >
          ...
          <button 
            ...
            className="w-full py-3 border-2 border-slate-800 text-slate-400 font-black uppercase text-[10px] rounded-2xl hover:bg-slate-900 transition-colors"
          >
            Nouvelle Vente
          </button>
        </div>
  ...
  return (
    <div className="bg-slate-900 p-8 rounded-3xl border border-emerald-900/20 shadow-2xl animate-in slide-in-from-left duration-300 w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <select 
            className="w-full bg-slate-950 border border-slate-800 p-4 rounded-2xl outline-none text-slate-300 focus:border-emerald-500 appearance-none mt-1"
        ...
        <div>
          ...
          <input type="text" placeholder="Nom complet du client" className="w-full bg-slate-950 border border-slate-800 p-4 rounded-2xl outline-none focus:border-emerald-500 text-white mt-1" 
        ...
        <div className="flex gap-4">
          <div className="w-1/3">
            ...
            <input 
              type="number" 
              className="w-full bg-slate-950 border border-slate-800 p-4 rounded-2xl outline-none focus:border-emerald-500 text-white mt-1" 
              min="1" 
          ...
          <div className="w-2/3">
            ...
            <input type="number" placeholder="0" className="w-full bg-slate-950 border border-slate-800 p-4 rounded-2xl outline-none focus:border-emerald-500 text-white mt-1" 
        ...
        <div className="bg-slate-950/50 p-4 rounded-2xl border border-slate-800 text-right">
          <span className="text-slate-500 text-[10px] uppercase mr-2 font-bold">Total à encaisser :</span>
          <span className="text-emerald-400 font-black text-xl">{(Number(saleData.quantite) * Number(saleData.prix_unitaire)).toLocaleString()} F</span>
        </div>

        <button 
          type="submit"
          disabled={selectedArt && selectedArt.quantite_stock < Number(saleData.quantite)}
          className={`w-full py-5 rounded-2xl font-black text-sm uppercase shadow-lg transition-all text-white ${
