...
function App() {
  ...
  const handleAddAchat = async (data) => {
    ...
    if (existing) {
      ...
      await supabase.from('articles').insert([{ user_id: session.user.id, nom: data.article_nom, quantite_stock: Number(data.quantite) }]);
    }
    fetchData(); 
  };
  ...
  return (
    <Router>
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center">
        {/* CSS DE NETTOYAGE POUR L'IMPRESSION PDF */}
        <style>{`
          @media print {
            .no-print, nav, footer, button { display: none !important; }
            body, .bg-slate-950 { background: white !important; color: black !important; }
        ...
        <main className="w-full max-w-2xl p-6 pb-24 animate-in fade-in duration-500 print-container">
          <Routes>
        ...
        <footer className="no-print mt-auto py-4 text-[8px] text-slate-700 uppercase tracking-[0.3em] font-bold text-center">
          Développé par SILVER'S DESIGN &copy; 2026
        </footer>
      </div>
    </Router>
  );
